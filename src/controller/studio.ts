...
export const createStudio:RequestHandler = async(req, res, next) => {
    ...
    await client.db('nextzy').collection('studio').insertOne({
      id: autoGenId,
      name: studio.name,
...
export const getAllStudio:RequestHandler = async(req, res, next) => {
  const client = new MongoClient(uri);
  await client.connect();
  const data = await client.db('nextzy').collection('studio').find({}).toArray();
  await client.close();
  if(data.length > 0) {
    res.status(200).send({
      "status": "success",
...
export const getStudioById:RequestHandler = async(req, res, next) => {
  ...
  if(!chkReg.test(parmas?.id)) {
    return res.status(400).json({"error": 'studioId is invalid (not uuid)'});
  }

  const client = new MongoClient(uri);
  await client.connect();
  await client.db('nextzy').collection('studio').findOne({
    id: parmas?.id
  }).then(async(resp) => {
    if(resp) {
      await client.close();
...
export const updateStudio: RequestHandler = async(req, res, next) => {
  const parmas: any = req.params;
  const chkReg = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i
  if(!chkReg.test(parmas?.id)) {
    return res.status(400).json({"error": 'studioId is invalid (not uuid)'});
  ...
  const client = new MongoClient(uri);
  await client.connect();
  await client.db('nextzy').collection('studio').findOneAndUpdate(
    { id: parmas?.id },
    {
      $set: {
...
export const deleteStudio: RequestHandler = async(req, res, next) => {
  const parmas: any = req.params;
  const chkReg = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i
  ...
  await client.db('nextzy').collection('studio').findOne({
    ...
    if(resp) {
      await client.db('nextzy').collection('studio').deleteOne({
        id: parmas?.id
      }).then(async(resDel) => {
        ...
        await client.db('nextzy').collection('anime').updateMany(
          { studioId: parmas?.id },
          {
            $set: {
              studioId: null
